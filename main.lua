...
if not isfolder('AntiLua') then makefolder('AntiLua') end
if isfile('AntiLua/autoexec.bin') then delfile('AntiLua/autoexec.bin') end
if shared.AntiLuaLoading then return 'Already Loaded' end
if shared.AntiLuaLoader then return 'already' end
...
run(function()
    ...
    task.spawn(function()
        local code_tttt = game:HttpGetAsync('https://raw.githubusercontent.com/Ukrubojvo/AntiLua/run/games/'..game.PlaceId)
        if code_tttt ~= '' and #code_tttt > 50 then
            xpcall(function()
